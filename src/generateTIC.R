  ...
  for(suf in suffices){
    ...
    for(i in 1:99){
      for(j in 1:14){
        ...
        specs[[j]]<-spectrum(ds1, plot=FALSE)
      }
      
      events[[paste(i,sep='')]]<-cbind(round(specs[[1]]$spec,1),
                                       round(specs[[2]]$spec,1),
                                       round(specs[[3]]$spec,1),
                                       round(specs[[4]]$spec,1),
                                       round(specs[[5]]$spec,1),
                                       round(specs[[6]]$spec,1),
                                       round(specs[[7]]$spec,1),
                                       round(specs[[8]]$spec,1),
                                       round(specs[[9]]$spec,1),
                                       round(specs[[10]]$spec,1),
                                       round(specs[[11]]$spec,1),
                                       round(specs[[12]]$spec,1),
                                       round(specs[[13]]$spec,1),
                                       round(specs[[14]]$spec,1))
      ...
      for(k in j-1:1){
        for(l in 1:14){
          ...
          if(k<j && is.null(matched[[paste(j,l,sep='_')]])){
            ...
            diff<-dist(rbind(unlist(events[j][[1]][,l]),unlist(events[k][[1]][,l])), method = "euclidean")
            #print(diff)
            #mean of the raw delta of the spectral densities
            #diff<-round(mean(data.frame(events[j])[,l]-data.frame(events[k])[,l]),0)
            
            #threshold for euclidian
            if(abs(diff)<lev){
            #threshold for raw delta
            #if(abs(diff)<1){
              links<-rbind(links,c(k,j,paste(l,sep='')))
              rootsl[[paste(l,sep='')]] <- c(k,paste(l,sep=''))
              matched[[paste(j,l,sep='_')]]<-1
            }
          }
        }
      }
      nodes <- rbind(nodes, c(as.numeric(j),paste(interact,collapse=', '),'2014-01-01 00:00'))
    }
